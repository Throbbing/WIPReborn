...
struct RBAABB
{
  ...
  bool is_contain(const RBVector3& v)
  {
  ...
  inline void fix_thin()
  {
    auto a = (max - min).get_abs();
    for (int i = 0; i < 3; ++i)
    {
      if (RBMath::is_nearly_zero(a[i]))
      {
        max[i] = min[i] + SMALL_F;
      }
    }
...
template <> struct TIsPODType<RBAABBI> { enum { v = true }; };

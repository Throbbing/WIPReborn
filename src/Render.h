...
*/
	...
	virtual void update_texture(WIPTexture2D* texture, int chanel,void* data) const = 0;
...
static unsigned int usual_unicode[] = {
...
class LargeTexture_TextRender : public WIPRender
{
public:
	LargeTexture_TextRender(int in_cache_w = 1024, int in_cache_h = 1024) :
		cache_w(in_cache_w), cache_h(in_cache_h){}
	virtual void init(const WIPCamera* cam = nullptr)
	{
		nx = 0;
		...
		text_lut_buffer = g_rhi->RHICreateTexture2D(cache_w, cache_h,0);
		text_cache[0] = new unsigned char[cache_w*cache_h];
		text_cache[1] = new unsigned char[cache_w*cache_h];
		text_cache[2] = new unsigned char[cache_w*cache_h];
		text_cache[3] = new unsigned char[cache_w*cache_h];
		memset(text_cache[0], 0, cache_h*cache_w);
		memset(text_cache[1], 0, cache_h*cache_w);
		memset(text_cache[2], 0, cache_h*cache_w);
		memset(text_cache[3], 0, cache_h*cache_w);

		text_cache_aos = new unsigned char[cache_w* cache_h* 4];
		memset(text_cache_aos, 0, cache_h*cache_w*4);
		...
		vf->add_float_vertex_attribute(2);
		vf->add_float_vertex_attribute(2);
		vf->add_float_vertex_attribute(1);

		bound_shader = g_rhi->RHICreateBoundShader(
			g_rhi->get_vertex_shader("text_vs1"), g_rhi->get_pixel_shader("text1")
			);
	...
	void update_cache(int chanel,int x, int y, int w, int h, unsigned char* data)
	{

		unsigned char* p = text_cache[chanel];
		p += x + y*cache_w;
		unsigned char* pp = data;
	...
	void load_text(const wchar_t* chs, int len = 3)
	{
		...
		for (int i = 0; i < len; ++i)
		{
			...
			update_cache(2,l, 0, width, height, bitmap.buffer);
			l += width;
			pack_texture();
			g_rhi->update_texture(text_lut_buffer,text_cache_aos);
		}
	}
	...
	{
		int baset = nx*ny;
		for (unsigned int i = 0; i < sizeof(usual_unicode); ++i)
		{
			FT_Glyph glyph;
			FT_Load_Glyph(ftface, FT_Get_Char_Index(ftface, usual_unicode[i]), FT_LOAD_RENDER);
			...
			if (c >= 4)
			{
				LOG_WARN("cache overflowed!");
				FT_Done_Glyph(glyph);
				return;
			}
			int level = used % baset;
			int x = (level % nx)*text_w;
			int y = (level / nx)*text_h;

			IndexStructure* p = new IndexStructure();
			p->chanel_id = c;
			p->id = usual_unicode[i];
			p->offx = offx;
			p->offy = offy;
			p->text_advance = advancex;
			p->text_height = height;
			p->text_width = width;
			p->texture_x = x;
			p->texture_y = y;
			index[usual_unicode[i]] = p;

			update_cache(c, x, y, width, height, bitmap.buffer);
	...
	void load_range(unsigned int begin,unsigned int end)
	{
		int baset = nx*ny;
		for (unsigned int i = begin; i <= end; ++i)
		{
			FT_Glyph glyph;
			FT_Load_Glyph(ftface, FT_Get_Char_Index(ftface, i), FT_LOAD_RENDER);
	...
	void pack_texture()
	{
		int total = cache_w*cache_h;
		for (int i = 0; i < total; ++i)
		{
			text_cache_aos[i * 4 + 0] = text_cache[0][i];
			text_cache_aos[i * 4 + 1] = text_cache[1][i];
			text_cache_aos[i * 4 + 2] = text_cache[2][i];
			text_cache_aos[i * 4 + 3] = text_cache[3][i];
		}
	}

	void build()
	{
		//basic Chinese 4e00-9fa5
		//extended Chinese 9fa6-9fcb
		//0-9 30-39
		//Lower case 61-7a
		//Capital 41-5a
		//load texts
		load_range(0x30, 0x39);
		
		load_range(0x61, 0x7a);
		load_range(0x41, 0x5a);
		
		//load_usual();
		//load_range(0x4e00, 0x9fa5);
		//load_range(0x9fa6, 0x9fcb);
		load_range(0x2e80, 0xfe4f);
		load_range(0xff00, 0xffef);
...
private:
	WIPTexture2D* text_lut_buffer;
	unsigned char* text_vertex_buffer;
	unsigned char* text_cache[4];
	unsigned char* text_cache_aos;
	int cache_w, cache_h;
	int nx, ny;
	int total, used;
	int text_w, text_h;
	...
	map<unsigned int, IndexStructure*> index;
